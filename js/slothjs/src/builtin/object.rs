...
fn object_object_defineProperty(
    ...
    let configurable = descriptor
        .get_value("configurable")
        .unwrap_or(&JSValue::from(false))
        .boolify(heap);
    let enumerable = descriptor
        .get_value("enumerable")
        .unwrap_or(&JSValue::from(false))
        .boolify(heap);
    let has_value =
        descriptor.get_value("value").is_some() || descriptor.get_value("writable").is_some();
    ...
    if has_get || has_set {
        ...
        if configurable {
            access = access | Access::CONF
        }
        if enumerable {
            access = access | Access::ENUM
        }
        if writable {
...
pub fn init(heap: &mut Heap) -> Result<JSRef, Exception> {
    ...
    *heap.get_mut(Heap::OBJECT_PROTO) = object_proto;
    ...
    object_object.set_hidden("getOwnPropertyDescriptor", Content::from(gop_ref))?;
